...
class TravauxDataPersister implements DataPersisterInterface
{
    private $em;
    private $serializer;
    ...
    {  
        ...
        if (is_null($Travaux->getDepartement()) && $PS->isEmpty() && is_null($SR)) {
            ...
            $Destinations = $PS->isEmpty() ? [] : $PS->map(function ($obj) {
                return $obj->getId(); })->getValues();
            ...
            if ($previousData) {
                ...
                foreach ($attributes as $attribute) {
                    ...
                    if (empty($previousData->$getter()) && empty($Travaux->$getter()))
                        continue;
                    if (
                    ...
                    if ($majorChange || $timeChange)
                        break;
                }
                // compare the collections
                $PPS = $previousData->getPs();
                $previousDestinations = $PPS->isEmpty() ? [] : $PPS->map(function ($obj) {
                    return $obj->getId(); })->getValues();
            ...
            if (is_null($Travaux->getDMS()) or $majorChange) { //||  depar, ps, source changed
                ...
                if (is_null($Travaux->getDepartement())) {
                    $Travaux->setDepartement(
                        is_null($SR) ? $PS->first()->getDepartement() : $SR->getDepartement()
                    );
                }
